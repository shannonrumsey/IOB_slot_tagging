...
class BiRNN(nn.Module):
    def __init__(self, vocab_size, embd_size, hidden_size, output_size, num_layers):
        ...
        self.rnn = nn.GRU(input_size=embd_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=0.1, bidirectional=True)
        self.fc = nn.Linear(hidden_size * 2, output_size)

    def forward(self, token_ids):
        embeddings = self.embedding(token_ids)
        rnn_out, _ = self.rnn(embeddings)
        preds = self.fc(rnn_out)
        return preds
...
model = BiRNN(vocab_size, embd_size, hidden_size, output_size, num_layers)
...
test_data = os.path.join(os.path.dirname(__file__), "hw2_test.csv")
...
saved_model = BiRNN(vocab_size, embd_size, hidden_size, output_size, num_layers)
